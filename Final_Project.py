...
def store_database(fav_playlist):
...
def favorite_artists(fav_playlist):
    artist_dict = {}
    tracks_data = sp.user_playlist(username, fav_playlist['id'],
                fields="tracks,next")
    more_tracks_data = tracks_data['tracks']
...
if __name__ == "__main__":
    ...
    if token:
        ...
        store_database(fav_playlist)
        
        return_list = favorite_artists(fav_playlist)
        sorted_list = sorted(return_list, key=lambda x: x[1], reverse=True)
        artist_list = []

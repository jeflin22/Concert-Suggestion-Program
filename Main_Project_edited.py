        ...
        if insert_count < 20:
            ...
            for event in pages:
                if insert_count < 20:
                    ...
                    lat = thing.latitude
                    lon = thing.longitude
                    coordinates = (lat,lon) #this is what I want
                    ...
                    cur.execute("CREATE TABLE IF NOT EXISTS Coordinates (latit text, longit text)")
                    cur.execute("INSERT INTO Coordinates(latit, longit) VALUES (?,?) ", (float(lat), float(lon)))
                else:
                    break
            ticketconn.commit()      
        else:
